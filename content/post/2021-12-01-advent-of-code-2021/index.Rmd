...
join_change_names <- function(x = out, iter_num = 1){ ## function to go into new room
...
1:20 %>% 
  reduce(.f = join_change_names, .init = first) %>% 
  unite(col = all, sep = "_", everything()) %>%
  filter(str_detect(all, "end")) %>%
  mutate(all = str_remove(all, pattern = "_end.*")) %>%
  mutate(all = str_c(all,"_end")) %>%
  distinct() %>%
  arrange(all)
...
```{r}
test_tens <- function(iteration = x) {
  out <-
    iteration %>%
    distinct(row, col, value) %>%
    select(row, col, value) %>%
    filter(value >= 10) %>% nrow()
  return(out)
}
